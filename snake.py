...
caption_height = 1000 #画布高度
white_color = (255, 255, 255) # 白色rgb
black_color = (0, 0, 0)
game_title = 'Pashazera'
cell = 10 # 格子
...
def draw_circular(color, position):
    pygame.draw.circle(caption, color, pygame.Rect(position[0], position[1], cell, cell))
...
def test():
    ...
    while True:
        ...
        if h + 15 > 1000:
            pygame.quit()
        pygame.draw.circle(caption, red_color, position, radius, width)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
